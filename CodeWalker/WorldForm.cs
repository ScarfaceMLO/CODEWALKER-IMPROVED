...
namespace CodeWalker
{
    public partial class WorldForm : Form, DXForm
    {
        ...
        MeshEditRenderer meshEditRenderer = null;
        bool isMeshEditMode = false;
        ...
        public WorldForm()
        {
            ...
            if (Settings.Default.ProjectWindowTheme == "Blue") // Default was Blue
            {
                Settings.Default.ProjectWindowTheme = "Dark";
            }
            SetTheme(Settings.Default.ProjectWindowTheme, false);
        }

        public void SetTheme(string themestr, bool changing = true)
        {
        ...
        private void Init()
        {
            ...
            KeyDown += WorldForm_KeyDown_Custom; // Added for mesh editing
            AllowDrop = true;
            DragEnter += WorldForm_DragEnter;
            DragDrop += WorldForm_DragDrop;
        ...
        private void UpdateControlInputs(float elapsed)
        {
            ...
            else
            {
                ...
                lock (MouseControlSyncRoot)
                {
                    ...
                    MouseLeftJustPressed = mlbjustpressed;
        ...
        private void RenderMoused()
        {
            ...
            if ((CurMouseHit.EntityDef != null) && (CurMouseHit.EntityDef.Archetype != null))
            {
                // check if the entity is already selected
                var sel = SelectedItem; // Snapshot for thread safety
                bool selected = (sel.EntityDef == CurMouseHit.EntityDef);
                if (!selected && (sel.MultipleSelectionItems != null))
                {
                    for (int i = 0; i < sel.MultipleSelectionItems.Length; i++)
                    {
                        if (sel.MultipleSelectionItems[i].EntityDef == CurMouseHit.EntityDef)
                        {
                            selected = true;
                            break;
                        }
                ...
                {
                    return; // Already selected, don't render mouse hit box
                }
            }
        ...
        private void RenderSelection()
        {
            ...
            if (sel.MultipleSelectionItems != null)
            {
                for (int i = 0; i < sel.MultipleSelectionItems.Length; i++)
                {
                    var item = sel.MultipleSelectionItems[i];
                    RenderSelection(ref item);
                }
            }
            else
            {
                RenderSelection(ref sel);
            }
        }
            ...
            if (mode == BoundsShaderMode.Box)
            {
                if ((ent != null) && (ent.Archetype != null))
                {
                    ...
                    if (draw != null)
                    {
                        var matrix = (ori.ToMatrix());
                        matrix.Row4 = new Vector4(camrel + camera.Position, 1.0f);
                        Renderer.RenderSelectionOverlay(draw, matrix, new Color4(0.0f, 1.0f, 0.0f, 0.25f)); // Transparent Green
                    }
                    else
                    {
        ...
        {
            if (!ShowWidget && !isMeshEditMode) return;

            if (isMeshEditMode && meshEditor != null && meshEditRenderer != null)
            {
                Renderer.RenderMeshEditor(meshEditor, meshEditRenderer);
            }
        ...
        private void UpdateWidgets()
        {
            if (isMeshEditMode)
            {
                //UpdateMeshEditInput(); // Handled in MouseDown
            }
        ...
        private void UpdateMouseHits()
        {
            if (isMeshEditMode)
            {
                //Mesh selection is handled in WorldForm_MouseDown (Right Click)
                //if (UpdateMeshEditInput())
                //{
                //    MouseLeftJustPressed = false; // Consume the click
                //}
                CurMouseHit.HitDist = float.MaxValue; // Ensure no entity is "hit" while in mesh edit mode
                return;
            }
        ...
        private void UpdateMouseHits(DrawableBase drawable, Archetype arche, YmapEntityDef entity)
        {
            ...
            if (scale.X != 0) invScale.X = 1.0f / scale.X;
            if (scale.Y != 0) invScale.Y = 1.0f / scale.Y;
            if (scale.Z != 0) invScale.Z = 1.0f / scale.Z;

            mraytrn = new Ray();
            mraytrn.Position = (orinv.Multiply(camera.MouseRay.Position - camrel)) * invScale;
            mraytrn.Direction = (orinv.Multiply(camera.MouseRay.Direction)) * invScale;
            
            // Normalize direction to ensure consistent results, but we need to account for scale in distance
            // Actually, if we use unnormalized direction scaled by invScale, 't' should remain world-space compatible?
            // Let's rely on t being the parameter. if D is scaled down, and O is scaled down, t should stay same.
                ...
                for (int i = 0; i < dmodels.Length; i++)
                {
                    ...
                    if (m.BoundsData != null)
                    {
                        ...
                        for (int j = 0; j < gbbcount; j++) //first box seems to be whole model
                        {
                            ...
                            bbox.Minimum = gbbox.Minimum;
                            bbox.Maximum = gbbox.Maximum;
                            bool usehit = false;
                            if (mraytrn.Intersects(ref bbox, out hitdist))
                            {
                                ...
                                if (hitdist > 0.0f) //firsthit || //ignore when inside the box
                                {
                                    ...
                                    float meshdist = geom.RayIntersect(mraytrn, Math.Min(CurMouseHit.HitDist, ghitdist));
                                    if (meshdist < float.MaxValue)
                                    {
                                        hitdist = meshdist;//use the mesh hit distance!
                    ...
                    else if (m.Geometries != null) // Fallback for missing BoundsData
                    {
                        for (int g = 0; g < m.Geometries.Length; g++)
                        {
                            var geom = m.Geometries[g];
                            float meshdist = geom.RayIntersect(mraytrn, Math.Min(CurMouseHit.HitDist, ghitdist));
                            if (meshdist < float.MaxValue)
                            {
                                hitdist = meshdist;
                                ...
                                if (CurMouseHit.Geometry != null)
                                {
                                    // Use archetype/drawable bbox as proxy since we don't have geometry bounds
                                    var b1 = (drawable.BoundingBoxMax - drawable.BoundingBoxMin) * scale;
                                    var b2 = (CurMouseHit.AABB.Maximum - CurMouseHit.AABB.Minimum) * scale;
                                    float r1 = b1.Length() * 0.5f;
                                    float r2 = b2.Length() * 0.5f;
                                    radsm = (r1 < (r2));
                                ...
                                if ((nearer && radsm) || radsm)
                                {
                                    ghitdist = hitdist;
                                    geometry = geom;
                                    geometryAABB.Minimum = drawable.BoundingBoxMin;
                                    geometryAABB.Maximum = drawable.BoundingBoxMax;
                                    geometryIndex = g;
                                }
                            }
                        }
        ...
        public void StartMeshEditMode(YdrFile ydr, MeshEditPanel panel, Matrix transform)
        {
            if (meshEditor == null)
            {
            ...
            if (meshEditRenderer == null)
            {
                meshEditRenderer = new MeshEditRenderer(Renderer);
            }

            if (meshEditor.StartEditing(ydr, transform))
            {
                isMeshEditMode = true;
                panel.SetMeshEditor(meshEditor);

                // Subscribe to widget events
                Widget.OnPositionChange -= Widget_OnPositionChange; // Unsubscribe default handler
                Widget.OnRotationChange -= Widget_OnRotationChange;
                Widget.OnScaleChange -= Widget_OnScaleChange;
                Widget.OnPositionChange += OnMeshWidgetPositionChange;
            }
            else
            {
                MessageBox.Show("Failed to start mesh editing mode", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        public void ExitMeshEditMode()
        {
            if (meshEditor != null)
            {
            ...
            bool wasEditing = isMeshEditMode;
            ...
            if (wasEditing)
            {
                ...
                Widget.OnPositionChange -= OnMeshWidgetPositionChange;
                Widget.OnPositionChange += Widget_OnPositionChange; // Restore default handler
                Widget.OnRotationChange += Widget_OnRotationChange;
                Widget.OnScaleChange += Widget_OnScaleChange;
                Widget.Visible = false;
            }

            if (ProjectForm != null && ProjectForm.MeshEditPanel != null)
            {
        ...
        {
            if (isMeshEditMode && meshEditor != null)
            {
                if (meshEditor.SelectedElements.Count > 0)
                {
        ...
        private void MarkUndoEnd(Widget w)
        {
            if (isMeshEditMode && meshEditor != null)
            {
                if (meshEditor.SelectedElements.Count > 0)
                {
                    var delta = Widget.Position - UndoStartPosition;
                    if (delta != Vector3.Zero)
                    {
                        var meshStep = new MeshTransformUndoStep(meshEditor, meshEditor.SelectedElements, delta);
                        RedoSteps.Clear();
                        UndoSteps.Push(meshStep);
                        UpdateUndoUI();
                    }
                }
                return;
        ...
        private void Save()
        {
            ...
            {
                SaveMeshYdr();
                return; // Don't save project if saving mesh explicitly
            }
            ProjectForm.Save();
        }
        ...
        private void SaveMeshYdr()
        {
            var ydr = meshEditor.CurrentYdr;
            if (ydr == null) return;

            // Force update buffers
            meshEditor.UpdateVertexBuffer();
            byte[] data = ydr.Save();
            ...
            sfd.FileName = Path.GetFileName(ydr.Name);
            sfd.Filter = "YDR Files|*.ydr";
            sfd.Title = "Save Modified Mesh YDR";
        ...
        private void WorldForm_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left)
            {
                ...
                if (GrabbedWidget != null)
                {
                    ...
                    if (isMeshEditMode && meshEditor != null)
                    {
                        meshEditor.CommitChanges();
                        // Update widget position to match new mesh center
                        var center = meshEditor.GetSelectionCenter();
                        var worldCenter = Vector3.TransformCoordinate(center, meshEditor.Transform);
                        GrabbedWidget.Position = worldCenter;
                    }
                    else if (SelectedItem.CanShowWidget)
                    {
        ...
        private void WorldForm_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (isMeshEditMode && meshEditor != null)
            {
                ...
                Ray ray = new Ray(camera.MouseRay.Position + camera.Position, camera.MouseRay.Direction);
                
                // Transform ray to local space
                var localRay = new Ray(
                    Vector3.TransformCoordinate(ray.Position, meshEditor.InverseTransform),
                    Vector3.TransformNormal(ray.Direction, meshEditor.InverseTransform)
                );
                localRay.Direction.Normalize();

                meshEditor.UpdateHover(localRay);
            }
        }
        ...
        private void WorldForm_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (!Input.kbmoving && !Widget.IsDragging) //don't trigger further actions if camera moving or widget dragging 
            {
                if (!ctrl)
                {
                    ...
                    if (k == Keys.Delete)
                    {
                        if (!(isMeshEditMode && meshEditor != null && meshEditor.CurrentMode == MeshEditMode.Face))
                        {
                            DeleteItem();
                        }
                    }
        ...
        private void WorldForm_DragEnter(object sender, DragEventArgs e)
        {
            if (e.Data.GetDataPresent(typeof(Archetype)))
            {
        ...
        private void WorldForm_DragDrop(object sender, DragEventArgs e)
        {
            try
            {
                CrashLogger.Log("[DRAG-DROP] WorldForm_DragDrop START");
                if (ProjectForm == null)
                {
                    CrashLogger.Log("[DRAG-DROP] ProjectForm is null, returning");
                    return;
                }
                if (e.Data.GetDataPresent(typeof(Archetype)))
                {
                    Archetype arch = (Archetype)e.Data.GetData(typeof(Archetype));
                    CrashLogger.Log($"[DRAG-DROP] Got archetype: {arch?.Name ?? "null"} (Hash: {arch?.Hash})");
                    
                    // Get screen coordinates from drag event
                    System.Drawing.Point screenPoint = new System.Drawing.Point(e.X, e.Y);
                    CrashLogger.Log($"[DRAG-DROP] Screen coordinates: ({screenPoint.X}, {screenPoint.Y})");
                    
                    // Convert to client coordinates
                    System.Drawing.Point p = PointToClient(screenPoint);
                    CrashLogger.Log($"[DRAG-DROP] Client coordinates: ({p.X}, {p.Y})");
                    CrashLogger.Log($"[DRAG-DROP] ClientSize: ({ClientSize.Width}, {ClientSize.Height})");
                    
                    // Calculate normalized mouse coordinates (same as Camera.SetMousePosition)
                    float mx = (p.X / (float)ClientSize.Width) * 2.0f - 1.0f;
                    float my = (p.Y / (float)ClientSize.Height) * -2.0f + 1.0f;
                    CrashLogger.Log($"[DRAG-DROP] Normalized mouse coords: ({mx}, {my})");
                    
                    // Calculate ray using the same method as Camera.UpdateProjection()
                    // These points are in CAMERA SPACE (relative to camera)
                    Vector3 rayNear = camera.ViewProjInvMatrix.MultiplyW(new Vector3(mx, my, 1.0f));
                    Vector3 rayFar = camera.ViewProjInvMatrix.MultiplyW(new Vector3(mx, my, 0.0f));
                    CrashLogger.Log($"[DRAG-DROP] RayNear (camera space): {rayNear}, RayFar (camera space): {rayFar}");
                    
                    // Ray in camera space (origin at camera)
                    Vector3 rayDir = Vector3.Normalize(rayFar - rayNear);
                    
                    // Transform to world space
                    Ray ray = new Ray(camera.Position, rayDir);
                    
                    CrashLogger.Log($"[DRAG-DROP] Ray origin (world): {ray.Position}, direction: {ray.Direction}");
                    CrashLogger.Log($"[DRAG-DROP] Space initialized: {space.Inited}, BoundsStore: {space.BoundsStore != null}");
                    CrashLogger.Log("[DRAG-DROP] Performing ray intersection");
                    
                    // Try collision raycast first
                    SpaceRayIntersectResult hit = space.RayIntersect(ray, 10000.0f);
                    Vector3 pos = Vector3.Zero;
                    
                    if (hit.Hit)
                    {
                        pos = hit.Position;
                        CrashLogger.Log($"[DRAG-DROP] Ray hit terrain at position: {pos}, distance: {hit.HitDist}");
                    }
                    else
                    {
                        // No collision geometry - intersect ray with horizontal plane below camera
                        float planeZ = camera.Position.Z - 10.0f; // 10 units below camera
                        if (Math.Abs(ray.Direction.Z) > 0.001f) // Avoid division by zero
                        {
                            ...
                            if (t > 0) // Only if intersection is in front of camera
                            {
                                ...
                                CrashLogger.Log($"[DRAG-DROP] Intersected with plane at Z={planeZ}: {pos}, distance: {t}");
                            }
                            else
                            {
                                // Ray pointing up - place at fixed distance
                                pos = ray.Position + (ray.Direction * 50.0f);
                                CrashLogger.Log($"[DRAG-DROP] Ray pointing up, placing at 50 units: {pos}");
                            }
                        }
                        else
                        {
                            // Ray parallel to ground - place at fixed distance
                            pos = ray.Position + (ray.Direction * 50.0f);
                            CrashLogger.Log($"[DRAG-DROP] Ray parallel to ground, placing at 50 units: {pos}");
                        }
                    }
                    
                    // Snap to ground with long-range vertical raycast
                    Ray groundRay = new Ray(pos, new Vector3(0, 0, -1.0f)); // Point down
                    SpaceRayIntersectResult groundHit = space.RayIntersect(groundRay, 1000.0f); // Search 1km down
                    
                    if (groundHit.Hit)
                    {
                        pos = groundHit.Position;
                        CrashLogger.Log($"[DRAG-DROP] Snapped to ground at: {pos}, drop distance: {groundHit.HitDist}");
                    }
                    else
                    {
                        // Try searching upward too
                        groundRay.Direction = new Vector3(0, 0, 1.0f); // Point up
                        groundHit = space.RayIntersect(groundRay, 100.0f); // Search 100 units up
                        if (groundHit.Hit)
                        {
                            pos = groundHit.Position;
                            CrashLogger.Log($"[DRAG-DROP] Snapped to ground above at: {pos}, rise distance: {groundHit.HitDist}");
                        }
                        else
                        {
                            CrashLogger.Log($"[DRAG-DROP] No ground found within range, keeping position: {pos}");
                        }
                    }

                    CrashLogger.Log("[DRAG-DROP] Calling ProjectForm.AddEntity");
                    ProjectForm.AddEntity(arch, pos);
                    CrashLogger.Log("[DRAG-DROP] AddEntity completed successfully");
                    
                    Activate(); // Ensure WorldForm keeps focus or regains it
                    CrashLogger.Log("[DRAG-DROP] WorldForm_DragDrop COMPLETE");
                }
            }
            catch (Exception ex)
            {
                CrashLogger.Log($"[DRAG-DROP] EXCEPTION in WorldForm_DragDrop: {ex.Message}");
                CrashLogger.Log($"[DRAG-DROP] Stack trace: {ex.StackTrace}");
                throw;
            }
        }

        private void OnMeshWidgetPositionChange(Vector3 newpos, Vector3 oldpos)
        {
            if (!isMeshEditMode || meshEditor == null) return;

            Vector3 delta = newpos - oldpos;
            
            // Transform delta to local space
            Vector3 localDelta = Vector3.TransformNormal(delta, meshEditor.InverseTransform);
            
            meshEditor.MoveSelectedElements(localDelta);
            meshEditor.UpdateVertexBuffer();

            if (meshEditor.CurrentYdr?.Drawable != null)
            {
                var renderable = Renderer.RenderableCache.GetRenderable(meshEditor.CurrentYdr.Drawable);
                if (renderable != null && renderable.IsLoaded)
                {
                     renderable.UpdateVertexData(Renderer.Device);
                }
            }
        }

        private void SelectMeshElement()
        {
            if (!isMeshEditMode || meshEditor == null) return;

            // Calculate ray in World Space (Right Click Selection)
            Ray ray = new Ray(camera.MouseRay.Position + camera.Position, camera.MouseRay.Direction);
            CrashLogger.Log($"[MESH-EDIT] RightClick WorldRay Origin: {ray.Position}, Dir: {ray.Direction}");
            ...
            CrashLogger.Log($"[MESH-EDIT] LocalRay Origin: {localRay.Position}, Dir: {localRay.Direction}");
            CrashLogger.Log($"[MESH-EDIT] MeshEditor Transform: {meshEditor.Transform}");

            bool multiSelect = (Control.ModifierKeys & Keys.Control) == Keys.Control;
            if (meshEditor.SelectElement(localRay, multiSelect))
            {
                CrashLogger.Log("[MESH-EDIT] Selection Successful!");
                // Update Gizmo center to selection center (World Space)
                var center = meshEditor.GetSelectionCenter();
                var worldCenter = Vector3.TransformCoordinate(center, meshEditor.Transform);
                
                Widget.Position = worldCenter;
                Widget.Rotation = Quaternion.RotationMatrix(meshEditor.Transform); // Align gizmo with entity
                Widget.Visible = true;
                Widget.Mode = WidgetMode.Position;
            }
            else
            {
                CrashLogger.Log("[MESH-EDIT] No element selected.");
            }
        }

        private void WorldForm_KeyDown_Custom(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Delete)
            {
                if (isMeshEditMode && meshEditor != null && meshEditor.CurrentMode == MeshEditMode.Face)
                {
                    Project.UndoStep undoStep = null;
                    try
                    {
                        undoStep = meshEditor.DeleteSelectedFaces();
                    }
                    catch (Exception ex)
                    {
                        string msg = $"[WorldForm] Error deleting faces: {ex}\n";
                    ...
                    if (undoStep != null)
                    {
                        ...
                        if (meshEditor.CurrentYdr?.Drawable != null)
                        {
                            ...
                            if (renderable != null && renderable.IsLoaded)
                            {
                                renderable.UpdateIndexData(Renderer.Device); 
                            }
                        }
                    }
                    e.Handled = true;
