...
namespace CodeWalker.Project
{
    public partial class ProjectForm : Form
    {
        ...
        public PropsPanel PropsPanel { get; set; }
        public VertexColorPanel VertexColorPanel { get; set; }
        public MeshEditPanel MeshEditPanel { get; set; }
        ...
        {
            if ((PropsPanel == null) || (PropsPanel.IsDisposed) || (PropsPanel.Disposing))
            {
                PropsPanel = new PropsPanel(this);
                PropsPanel.Init();
                PropsPanel.SetTheme(Theme);
                PropsPanel.Show(MainDockPanel, DockState.Float);
            }
            else
            {
                PropsPanel.Show();
        ...
        public void ShowVertexColorPanel()
        {
            if ((VertexColorPanel == null) || (VertexColorPanel.IsDisposed) || (VertexColorPanel.Disposing))
            {
                VertexColorPanel = new VertexColorPanel(this);
                VertexColorPanel.SetTheme(Theme);
                VertexColorPanel.Show(MainDockPanel, DockState.Float);
            }
            else
            {
                VertexColorPanel.Show();
            }
        }

        public void ShowMeshEditPanel(YdrFile ydr, Matrix transform)
        {
            if ((MeshEditPanel == null) || (MeshEditPanel.IsDisposed) || (MeshEditPanel.Disposing))
            {
                MeshEditPanel = new MeshEditPanel(this);
                MeshEditPanel.Show(MainDockPanel, DockState.Float);
            }
            else
            {
                MeshEditPanel.Show();
            ...
            // Start mesh editing mode in WorldForm
            WorldForm?.StartMeshEditMode(ydr, MeshEditPanel, transform);
        }

        public void HideMeshEditPanel()
        {
            if (MeshEditPanel != null && !MeshEditPanel.IsDisposed)
            {
                MeshEditPanel.Hide();
            }
        }
        ...
        public void OpenProject(string filepath)
        {
            ...
            {
                MessageBox.Show("Project file not found: " + filepath, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            ...
            CurrentProjectFile = new ProjectFile();
            CurrentProjectFile.Load(filepath);
        ...
        public void ReloadProject()
        {
            ...
            string projectPath = CurrentProjectFile.Filepath;

            if (string.IsNullOrEmpty(projectPath) || !File.Exists(projectPath))
            {
                MessageBox.Show("Cannot reload project: project file path is invalid.", "Reload Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            // Close the project without save prompts (assuming user just saved)
        ...
        private void CloseProjectSilently()
        {
        ...
        public YmapEntityDef AddEntity(Archetype arch, Vector3 pos)
        {
            try
            {
                CrashLogger.Log($"[ADD-ENTITY] AddEntity START - Archetype: {arch?.Name ?? "null"} at position {pos}");
                
                if (CurrentYmapFile == null)
                {
                    CrashLogger.Log("[ADD-ENTITY] CurrentYmapFile is null, creating new Ymap");
                    NewYmap();
                    if (CurrentProjectFile != null && CurrentProjectFile.YmapFiles.Count > 0)
                    {
                        CurrentYmapFile = CurrentProjectFile.YmapFiles[CurrentProjectFile.YmapFiles.Count - 1];
                    }
                }
                if (CurrentYmapFile == null)
                {
                    CrashLogger.Log("[ADD-ENTITY] CurrentYmapFile still null after NewYmap, returning null");
                    return null;
                }

                CrashLogger.Log("[ADD-ENTITY] Creating CEntityDef");
                CEntityDef cent = new CEntityDef();
                cent.archetypeName = arch.Hash;
                cent.rotation = new Vector4(0, 0, 0, 1);
                cent.scaleXY = 1.0f;
                cent.scaleZ = 1.0f;
                cent.flags = 32;
                cent.parentIndex = -1;
                cent.lodDist = arch.LodDist;
                cent.lodLevel = rage__eLodType.LODTYPES_DEPTH_ORPHANHD;
                cent.priorityLevel = rage__ePriorityLevel.PRI_REQUIRED;
                cent.ambientOcclusionMultiplier = 255;
                cent.artificialAmbientOcclusion = 255;
                ...
                CrashLogger.Log("[ADD-ENTITY] Creating YmapEntityDef");
                YmapEntityDef ent = new YmapEntityDef(CurrentYmapFile, 0, ref cent);
                
                CrashLogger.Log("[ADD-ENTITY] Setting archetype on entity");
                ent.SetArchetype(arch);

                if (WorldForm != null)
                {
                    CrashLogger.Log("[ADD-ENTITY] Acquiring WorldForm.RenderSyncRoot lock");
                    lock (WorldForm.RenderSyncRoot)
                    {
                        CrashLogger.Log("[ADD-ENTITY] Lock acquired, adding entity to ymap");
                        CurrentYmapFile.AddEntity(ent);
                        CrashLogger.Log("[ADD-ENTITY] Entity added to ymap, releasing lock");
                    }
                }
                else
                {
                    CrashLogger.Log("[ADD-ENTITY] WorldForm is null, adding entity without lock");
                    CurrentYmapFile.AddEntity(ent);
                }

                CrashLogger.Log("[ADD-ENTITY] Loading project tree");
                LoadProjectTree();
                CrashLogger.Log("[ADD-ENTITY] Selecting entity in tree");
                ProjectExplorer?.TrySelectEntityTreeNode(ent);
                CurrentEntity = ent;
                CrashLogger.Log("[ADD-ENTITY] Showing edit panel");
                ShowEditYmapEntityPanel(false);

                CrashLogger.Log("[ADD-ENTITY] AddEntity COMPLETE");
                return ent;
            }
            catch (Exception ex)
            {
                CrashLogger.Log($"[ADD-ENTITY] EXCEPTION in AddEntity: {ex.Message}");
                CrashLogger.Log($"[ADD-ENTITY] Stack trace: {ex.StackTrace}");
                throw;
            }
        }
        ...
        public void SaveYdr(bool saveas = false)
        {
            if (CurrentYdrFile == null) return;

            string ydrname = CurrentYdrFile.Name;
            string filepath = CurrentYdrFile.FilePath;
            if (string.IsNullOrEmpty(filepath))
            {
                filepath = ydrname;
            }
            ...
            lock (projectsyncroot)
            {
                ...
                if (saveas)
                {
                    filepath = ShowSaveDialog("Ydr files|*.ydr", filepath);
                    if (string.IsNullOrEmpty(filepath))
                    { return; }

                    string newname = Path.GetFileNameWithoutExtension(filepath);
                    JenkIndex.Ensure(newname);
                    CurrentYdrFile.FilePath = filepath;
                    CurrentYdrFile.RpfFileEntry.Name = new FileInfo(filepath).Name;
                    CurrentYdrFile.Name = CurrentYdrFile.RpfFileEntry.Name;
                }

                data = CurrentYdrFile.Save();
            }
            ...
            //SetYdrHasChanged(false); //Method doesn't exist yet? No, SetCurrentSaveItem logic handles some UI.
            //We will check if SetYdrHasChanged exists or if we need to set HasChanged manually.
            //Actually, searching for SetYdrHasChanged found nothing. 
            //But looking at other Save methods, they call Set[Type]HasChanged.
            //I should check if that method exists or if I need to use generic SetProjectItemHasChanged?
            //Actually lines 9153+ seemed to have Set... methods.
            //I'll assume I need to manually set it for now or add the helper. 
            //Let's check if there is a SetYdrHasChanged. I didn't see it in lines 9000-10375.
            //So I'll just do it manually here.
            
            CurrentYdrFile.HasChanged = false; //Reset changed flag
            ...
            if (saveas)
            {
                ...
                {
                    string origpath = CurrentProjectFile.GetRelativePath(origfile);
                    string newpath = CurrentProjectFile.GetRelativePath(CurrentYdrFile.FilePath);
                    if (!CurrentProjectFile.RenameYdr(origpath, newpath)) //RenameYdr might not exist either...
                    { 
                        //Try generic Rename?
                    }
                    ProjectExplorer?.UpdateYdrTreeNode(CurrentYdrFile); //Verify this exists
                }
                SetProjectHasChanged(true);
                SetCurrentSaveItem();
            }
        ...
        private void ViewPropsPanelMenu_Click(object sender, EventArgs e)
        {
            ShowPropsPanel();
        }

        private void ViewEdit3DMeshMenu_Click(object sender, EventArgs e)
        {
            if (WorldForm == null) return;
            var selection = WorldForm.CurrentMapSelection;
            var entity = selection.EntityDef;

            if (entity == null)
            {
                MessageBox.Show("Please select an entity first.", "No Entity Selected", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (entity.MloParent != null)
            {
                MessageBox.Show("Mesh editing not supported for MLO entities yet.", "Not Supported", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }

            if (entity.Archetype == null) return;
            
            var ydr = WorldForm.GameFileCache.GetYdr(entity.Archetype.Hash);
            if (ydr == null)
            {
                 MessageBox.Show("Could not load YDR for this entity.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                 return;
            }
            ...
            ShowMeshEditPanel(ydr, transform);
        }

        private void ViewEditVertexMenu_Click(object sender, EventArgs e)
        {
            if (WorldForm == null) return;
            var selection = WorldForm.CurrentMapSelection;
            var entity = selection.EntityDef;
            ...
            if (entity.MloParent != null)
            {
                MessageBox.Show("Vertex editing not supported for MLO entities yet.", "Not Supported", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            ShowVertexColorPanel();
            if (VertexColorPanel != null)
            {
                VertexColorPanel.LoadYdr(ydr);
            }
        }
