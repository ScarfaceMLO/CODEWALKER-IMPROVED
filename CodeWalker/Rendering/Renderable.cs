...
namespace CodeWalker.Rendering
{
    ...
    public class Renderable : RenderableCacheItem<DrawableBase>
    {
        ...
        // [New] Added for Mesh Editing - Vertex Buffer update
        public void UpdateVertexData(Device device)
        {
            if (AllModels != null)
            {
                foreach (var model in AllModels)
                {
                    ...
                    foreach (var geom in model.Geometries)
                    {
                        geom.UpdateVertexData(device);
                    }
                }
            }
        }

        public void UpdateIndexData(Device device)
        {
            if (AllModels != null)
            {
                foreach (var model in AllModels)
    ...
    public class RenderableGeometry
    {
        ...
        private class PendingIndexUpdate { public Buffer Buffer; public int Count; }
        private class PendingVertexUpdate { public Buffer Buffer; public VertexBufferBinding Binding; }
        private PendingIndexUpdate _pendingIndexUpdate;
        private PendingVertexUpdate _pendingVertexUpdate;
        ...
        public void UpdateVertexData(Device device)
        {
            ...
            VertexBufferBinding newBinding = default(VertexBufferBinding);

            try
            {
                if ((DrawableGeom?.VertexData?.VertexBytes != null) && 
                    (DrawableGeom.VertexData.VertexBytes.Length > 0) &&
                    (device != null))
                {
                    newBuffer = Buffer.Create(device, BindFlags.VertexBuffer, DrawableGeom.VertexData.VertexBytes);
                    if (newBuffer != null)
                    {
                        newBinding = new VertexBufferBinding(newBuffer, VertexStride, 0);
                    }
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[UpdateVertexData] Error creating buffer: {ex.Message}");
            }

            // Atomic update of pending buffer
            var update = new PendingVertexUpdate { Buffer = newBuffer, Binding = newBinding };
            var oldUpdate = System.Threading.Interlocked.Exchange(ref _pendingVertexUpdate, update);
            
            // If there was already a pending update that hasn't been picked up by Render, dispose its buffer immediately.
            if ((oldUpdate != null) && (oldUpdate.Buffer != null))
            {
                oldUpdate.Buffer.Dispose();
            }
        }
        ...
        public void UpdateIndexData(Device device)
        {
            ...
            try
            {
                if ((DrawableGeom?.IndexBuffer?.Indices != null) && 
                    (DrawableGeom.IndexBuffer.Indices.Length > 0) && 
                    (device != null))
                {
                    newBuffer = Buffer.Create(device, BindFlags.IndexBuffer, DrawableGeom.IndexBuffer.Indices);
                    newCount = (int)DrawableGeom.IndicesCount; 
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[UpdateIndexData] Error creating buffer: {ex.Message}");
            }

            // Atomic update of pending buffer
            var update = new PendingIndexUpdate { Buffer = newBuffer, Count = newCount };
            var oldUpdate = System.Threading.Interlocked.Exchange(ref _pendingIndexUpdate, update);
        ...
        {
            // COMMIT PENDING UPDATES (Render Thread Only)
            var idxUpdate = System.Threading.Interlocked.Exchange(ref _pendingIndexUpdate, null);
            if (idxUpdate != null)
            {
                var old = IndexBuffer;
                IndexBuffer = idxUpdate.Buffer;
                IndexCount = idxUpdate.Count;
                
                if (old != null)
                {
                    var buf = old;
                    Task.Delay(2000).ContinueWith(t => { try { buf.Dispose(); } catch { } });
                }
            }

            var vtxUpdate = System.Threading.Interlocked.Exchange(ref _pendingVertexUpdate, null);
            if (vtxUpdate != null)
            {
                var old = VertexBuffer;
                VertexBuffer = vtxUpdate.Buffer;
                VBBinding = vtxUpdate.Binding;
        ...
        {
             // COMMIT PENDING UPDATES (Render Thread Only) - Duplicated logic
            var idxUpdate = System.Threading.Interlocked.Exchange(ref _pendingIndexUpdate, null);
            if (idxUpdate != null)
            {
                var old = IndexBuffer;
