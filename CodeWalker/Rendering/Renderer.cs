...
namespace CodeWalker.Rendering
{
    public class Renderer
    {
        ...
        private void UpdateGlobalLights()
        {
            ...
            else
            {
                ...
                if ((weather != null) && weather.Inited)
                {
                    ...
                    // High Fidelity Lighting Boost
                    lightdircolour *= Math.Max(lightdircolour.Alpha, minmult) * 1.3f; 
                    lightdirambcolour *= lightdirambcolour.Alpha * lamult * 1.2f;
        ...
        public void RenderMeshEditor(MeshEditor editor, MeshEditRenderer renderer)
        {
            renderer.Render(context, camera, editor);
        }
        ...
        public void RenderSelectionOverlay(DrawableBase drawable, Matrix transform, Color4 col)
        {
            if (drawable?.DrawableModels?.High == null) return;
            
            // Set render states for transparent overlay
            shaders.SetDepthStencilMode(context, DepthStencilMode.DisableWrite);
            shaders.SetRasterizerMode(context, RasterizerMode.SolidDblSided);
            shaders.SetDefaultBlendState(context); // Use standard alpha blending
            
            // We need to accumulate vertices for batch rendering if possible, but here we'll just process geometry by geometry
            // A better way for performance might be to collecting all triangles first.
            
            List<VertexTypePC> verts = new List<VertexTypePC>();
            uint c = (uint)col.ToRgba();
            ...
            foreach (var model in drawable.DrawableModels.High)
            {
                ...
                {
                    if (geom.IndexBuffer?.Indices == null || geom.VertexData == null) continue;
                    var indices = geom.IndexBuffer.Indices;
                    
                    for (int i = 0; i < indices.Length; i++)
                    {
                        var idx = indices[i];
                        var v = geom.VertexData.GetVector3(idx, 0);
                        
                        verts.Add(new VertexTypePC() { Position = v, Colour = c });
                    }
                }
            }
            ...
            if (verts.Count > 0)
            {
                RenderFilledTriangles(verts, transform, DepthStencilMode.DisableWrite);
            }
        ...
        public void RenderSelectionWireframe(DrawableBase drawable, Matrix transform, uint col)
        {
            if (drawable?.DrawableModels?.High == null) return;
            foreach (var model in drawable.DrawableModels.High)
            {
                ...
                foreach (var geom in model.Geometries)
                {
                    ...
                    {
                        var v0 = geom.VertexData.GetVector3(indices[i], 0);
                        var v1 = geom.VertexData.GetVector3(indices[i + 1], 0);
                        var v2 = geom.VertexData.GetVector3(indices[i + 2], 0);

                        Vector4 v0t, v1t, v2t;
                        Vector3.Transform(ref v0, ref transform, out v0t);
                        Vector3.Transform(ref v1, ref transform, out v1t);
                        Vector3.Transform(ref v2, ref transform, out v2t);

                        Vector3 p0 = new Vector3(v0t.X, v0t.Y, v0t.Z);
                        Vector3 p1 = new Vector3(v1t.X, v1t.Y, v1t.Z);
                        Vector3 p2 = new Vector3(v2t.X, v2t.Y, v2t.Z);

                        RenderSelectionLine(p0, p1, col);
                        RenderSelectionLine(p1, p2, col);
                        RenderSelectionLine(p2, p0, col);
                    }
                }
            }
        }
        ...
        private Renderable TryGetRenderable(Archetype arche, DrawableBase drawable, uint txdHash = 0, TextureDictionary txdExtra = null, Texture diffOverride = null)
        {
            ...
            if (arche != null)
            {
                if (arche.TextureDict.Hash != 0) texDict = arche.TextureDict.Hash;
        ...
        public void RenderLines(IEnumerable<VertexTypePC> verts, Matrix transform)
        {
            ...
            if (!transform.IsIdentity)
            {
                ...
                {
                    vList[i] = new VertexTypePC() 
                    { 
                        Position = Vector3.TransformCoordinate(vList[i].Position, transform), 
                        Colour = vList[i].Colour 
                    };
                }
            }
            
            RenderLines(vList, DepthStencilMode.DisableAll);
        }

        public void RenderFilledTriangles(IEnumerable<VertexTypePC> verts, Matrix transform, DepthStencilMode dsmode = DepthStencilMode.Enabled)
        {
            var vList = verts.ToList();
            if (transform != Matrix.Identity)
            {
                ...
                {
                    v = vList[i];
                    v.Position = (Vector3)Vector3.Transform(v.Position, transform);
                    vList[i] = v;
                }
            }
            shaders.SetDepthStencilMode(context, dsmode);
            shaders.Paths.RenderTriangles(context, vList, camera, shaders.GlobalLights);
        }

        public void RenderTriangles(IEnumerable<VertexTypePC> verts, Matrix transform)
        {
            var vList = verts.ToList();
            var lList = new List<VertexTypePC>();
            for(int i=0; i<vList.Count; i+=3)
            {
                if (i+2 >= vList.Count) break;
                lList.Add(vList[i]); lList.Add(vList[i+1]);
                lList.Add(vList[i+1]); lList.Add(vList[i+2]);
                lList.Add(vList[i+2]); lList.Add(vList[i]);
            }
            RenderLines(lList, transform);
        }

        public void RenderSelectionSphere(BoundingSphere sphere, Color4 color)
        {
            var min = sphere.Center - new Vector3(sphere.Radius);
            var max = sphere.Center + new Vector3(sphere.Radius);
            var col = (uint)color.ToRgba();
            
            var v0 = new Vector3(min.X, min.Y, min.Z);
            var v1 = new Vector3(max.X, min.Y, min.Z);
            var v2 = new Vector3(max.X, max.Y, min.Z);
            var v3 = new Vector3(min.X, max.Y, min.Z);
            var v4 = new Vector3(min.X, min.Y, max.Z);
            var v5 = new Vector3(max.X, min.Y, max.Z);
            var v6 = new Vector3(max.X, max.Y, max.Z);
            var v7 = new Vector3(min.X, max.Y, max.Z);

            var lList = new List<VertexTypePC>();
            
            lList.Add(new VertexTypePC() { Position = v0, Colour = col }); lList.Add(new VertexTypePC() { Position = v1, Colour = col });
            lList.Add(new VertexTypePC() { Position = v1, Colour = col }); lList.Add(new VertexTypePC() { Position = v2, Colour = col });
            lList.Add(new VertexTypePC() { Position = v2, Colour = col }); lList.Add(new VertexTypePC() { Position = v3, Colour = col });
            lList.Add(new VertexTypePC() { Position = v3, Colour = col }); lList.Add(new VertexTypePC() { Position = v0, Colour = col });

            lList.Add(new VertexTypePC() { Position = v4, Colour = col }); lList.Add(new VertexTypePC() { Position = v5, Colour = col });
            lList.Add(new VertexTypePC() { Position = v5, Colour = col }); lList.Add(new VertexTypePC() { Position = v6, Colour = col });
            lList.Add(new VertexTypePC() { Position = v6, Colour = col }); lList.Add(new VertexTypePC() { Position = v7, Colour = col });
            lList.Add(new VertexTypePC() { Position = v7, Colour = col }); lList.Add(new VertexTypePC() { Position = v4, Colour = col });

            lList.Add(new VertexTypePC() { Position = v0, Colour = col }); lList.Add(new VertexTypePC() { Position = v4, Colour = col });
            lList.Add(new VertexTypePC() { Position = v1, Colour = col }); lList.Add(new VertexTypePC() { Position = v5, Colour = col });
            lList.Add(new VertexTypePC() { Position = v2, Colour = col }); lList.Add(new VertexTypePC() { Position = v6, Colour = col });
            lList.Add(new VertexTypePC() { Position = v3, Colour = col }); lList.Add(new VertexTypePC() { Position = v7, Colour = col });

            RenderLines(lList, Matrix.Identity);
        }
    }
